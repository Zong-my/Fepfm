...
class GetUsefulData:
    ...
    def get_sys_data(self):
        ...
        temp_system['time'] = pd.to_datetime(temp_system['time'].astype(str), format='%Y%m%d%H%M')
        # convert all values in the 'time' column to integer multiples of 10min
        logging.info("convert all values in the 'time' column to integer multiples of 10min..........")
        temp_system['time'] = temp_system['time'].dt.ceil('10min').dt.strftime("%Y-%m-%d %H:%M")
        # 
    ...
    def complete_timestamp(self, temp_system):
        ...
        return (
            temp_system.set_index('time')
            .reindex(standard_time)
            .rename_axis('time')
            .reset_index()
        )
    ...
    def adp_1(self, data):
        ...
        for i in range(len(columns)):
            if columns[i] in ['dry_tmp', 'wet_tmp', 'T1', 'T2', 'Y']:
                ...
                pass
        if 'Y' in df.columns:
            df['Y'] = np.where((df['Y'] > 50000), np.nan, df['Y'])
        df = df.reset_index()
        return df
