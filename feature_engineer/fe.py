...
class AddFeatures:
    ...
    def add_basic_time_features(self, data, add_name=''):
        ...
        df['year' + add_name] = index.year
        df['month' + add_name] = month
        df['day' + add_name] = index.day
        df['day_of_year' + add_name] = index.dayofyear
        df['week_of_year' + add_name] = iso_week
        df['quarter' + add_name] = index.quarter
        df['season' + add_name] = month % 12 // 3 + 1
        df['week_day' + add_name] = weekday
        df['hour_of_day' + add_name] = index.hour
        df['minute' + add_name] = index.minute
        df['month_sin' + add_name] = np.sin(2 * np.pi * month / 12)
        df['month_cos' + add_name] = np.cos(2 * np.pi * month / 12)
        df['week_sin' + add_name] = np.sin(2 * np.pi * iso_week / 52)
        df['week_cos' + add_name] = np.cos(2 * np.pi * iso_week / 52)
        df['weekday_sin' + add_name] = np.sin(2 * np.pi * weekday / 7)
        df['weekday_cos' + add_name] = np.cos(2 * np.pi * weekday / 7)
        df['hour_sin' + add_name] = np.sin(2 * np.pi * index.hour / 24)
        df['hour_cos' + add_name] = np.cos(2 * np.pi * index.hour / 24)
        df['minute_sin' + add_name] = np.sin(2 * np.pi * index.minute / 60)
        df['minute_cos' + add_name] = np.cos(2 * np.pi * index.minute / 60)
        df['if_weekend' + add_name] = (weekday > 4).astype(int)
        df['if_workday' + add_name] = pd.Series(index.date, index=index).map(
            lambda x: int(self.judge_workday(x))
        ).to_numpy()
    ...
    def add_next_day_basic_time_features(self, ori_data, next_date, freq='10min'):
        time_to_add = pd.date_range(pd.to_datetime(next_date), pd.to_datetime(next_date) + timedelta(days=1),
                                    freq=freq, inclusive='left')  # .strftime('%Y-%m-%d %H:%M:%S').to_list()
